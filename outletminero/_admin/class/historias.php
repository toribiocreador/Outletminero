	<?php
echo (preg_match('/historias.php/',$_SERVER['SCRIPT_NAME'])) ? header('Location: ../') : '';
class historias extends _GLOBAL_{
...
public function addMod(){
	...
	if (isset($_GET["id"])){
		if($_SESSION["nivel"]=='6')
			...
			$socio = '0';
		$db = $this->_db();
		$result = mysql_query("SELECT * FROM historias WHERE id='{$_GET['id']}' AND socio='{$socio}'");
		$db = $this->_db();
	...
	return '<p class="txtTitles">Agregar Historia</p>
<p class="txtCont1">Para dar de alta tu publicaci&oacute;n selecciona lo siguiente:</p>
<p class="txtCont1">&nbsp;</p>

<form id="form" name="form" enctype="multipart/form-data" method="post" action="?F=historias&amp;_f=addHistoria">
...
public function addHistoria(){
	...
	$ruta = 'gallery/historias/';
	$ruta2 = 'gallery/historias/galeria/';
	$imagen = '';
	$producto = '';
	...
	$db = $this->_db();
	if ($_POST["id"] == 0){
		mysql_query("INSERT INTO historias (titulo, introduccion, contenido, embed, imagen, pf, fechapub, autor, socio)
					VALUES ('". $titulo ."', '". $introduccion ."', '". $contenido ."', '". $embed ."', '". $imagenes ."', '". $pf ."', '". $fechapub ."', '". $autor ."', '". $socio ."')") or die(mysql_error());
		$id=mysql_insert_id();
	} else {
		$sql = "UPDATE historias SET titulo='$titulo', introduccion='$introduccion', contenido='$contenido', embed='$embed', imagen='$imagenes', pf='$pf', fechapub='$fechapub', autor='$autor', socio='$socio' WHERE id='{$_POST['id']}'";
...
public function seeDelete(){
	if($_SESSION["nivel"]=='6')
		...
		$socio = '0';
	$db = $this->_db();
	$result = mysql_query("SELECT * FROM historias WHERE socio='$socio' ORDER BY id DESC");
	$echo = '';
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			$echo .= '<tr>
						<td style="background:#F7F7F7" width="30" align="center"><a href="?F=historias&amp;_f=addMod&amp;id='. $row["id"] .'"><img src="imgs/img_edit.png" border="0" /></a></td>
						<td style="background:#F7F7F7" class="linkCont"><a href="?F=historias&amp;_f=addMod&amp;id='. $row["id"] .'">'. $row["titulo"] .' </a></td>
						...
						<td style="background:#F7F7F7" width="30" align="center"><a href="javascript:void(ConfirmDelete(\'?F=historias&amp;_f=delHistoria&amp;id='. $row["id"] .'\'))"><img src="imgs/img_delete.png" border="0" /></a></td>
					  </tr>';
	}//while
	$content = '<p class="txtTitles">Listado de Historias</p>
...
public function delHistoria(){
	...
	$db=$this->_db();
	$result = mysql_query("SELECT * FROM historias WHERE id='{$_GET['id']}'");
	$row = mysql_fetch_array($result, MYSQL_ASSOC);
	$tok=explode("/",$row["imagen"]);
	if($tok[1]=="historias")
	@unlink('../'. $row["imagen".$i]);
	...
	mysql_query("DELETE FROM historias WHERE id='{$_GET['id']}'");
	$echo ='<script type="text/javascript">
				setTimeout("alert(\'EL REGISTRO HA SIDO ELIMINADO\');",100); 
				setTimeout("top.location.href = \'?F=historias&_f=seeDelete\'",1000);
				</script>';
	return $echo;
}//delete
