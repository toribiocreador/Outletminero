...
public function main(){
	...
	$result = mysql_query("SELECT * FROM entrevistas WHERE activo=1 ORDER BY id DESC ". $qLimit);
	$content = '<p class="txtTit">Entrevistas</p>';
	return $content . $this->showPublicacion($result, 'entrevistas', '3') . $this->paginacion($_GET["ver"], $this->PAGINAS, $this->LIMIT, 'entrevistas', 'entrevistas', 'main', 1);
}//main
...
public function ver(){
	...
	$this -> lectura("entrevistas",$_GET["id"]);
	$db = $this->_db();
	...
	// Cantidad de comentarios ....................
		$result2 = mysql_query("SELECT * FROM comentarios WHERE seccion='3' AND id_publicacion='{$row['id']}' ORDER BY id DESC");
		$totalComm = (mysql_num_rows($result2) < 1) ? 0 : mysql_num_rows($result2);
		mysql_free_result($result2);
	$pie = '';
	if($row["pf"]!='')
		$pie = '<span class="txtPP" style="padding:5px;>'. $row["pf"] .'</span>';
	$img = (isset($tok[0]) && $tok[0] != '') ? '<div style="border:#000000 1px solid; background:#FFFFFF; float:left; font-size:10px; color:#333333; margin-right: 20px; width:auto;"><img src="'. $tok[0] .'" width=300" alt="" border="0" /><br>'.$pie.'</span>'.$this->slideGalerias('entrevistas',$row["id"]).'</div>':'<iframe width="640" height="390" src="'. $row["embed"] .'" frameborder="0" allowfullscreen></iframe>';
...
<script type="text/javascript">stLight.options({publisher: \'604ae005-d084-4823-8650-44bf570d6993\',
	tracking:\'google\', headerTitle:\'Outletminero\', popup: \'true\'}); </script>
          ...
          <p class="linkCont"><a href="?F=comentarios&amp;_f=main&amp;tipo=3&amp;id='. $row["id"] .'">Deja tu comentario</a> <span class="linkCont"><a href="?F=inicio&amp;_f=verNormas">Normas de uso</a></span></p>'. $this->showComments('3', $row["id"]);
}//ver
