...
public function main(){
	...
	if (isset($_SESSION["nombre"]) && isset($_SESSION["autorizado"])) {
		$nombre = '<input id="nombre" name="nombre" type="text" size="60" maxlength="200" readonly="true" class="frmInputM" value="'. $_SESSION["nombre"] .'"/>';
		$idusuario = $_SESSION["idusuario"];
	} else {
		$nombre ='<input id="nombre" name="nombre" type="text" size="60" maxlength="200" class="frmInputM" />';
		$idusuario = '0';
	}//if
	...
	return '<p class="txtTit">D&eacute;janos tu comentario</p>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="3">
            <tr>
              <td class="txtCont">Publicado el<span class="txtBold"> '. $this->txtFechaPub($row["fechapub"]) .'</span>, En <span class="linkCont"><a href="?F='. $this->tabla[$_GET["tipo"]] .'&amp;_f=main">'. $this->seccion[$_GET["tipo"]].'</a></span></td>
            </tr>
          </table>
		  ...
		  <form id="frmComentario" name="frmComentario" method="post" action="?F=comentarios&amp;_f=add">
			...
			<input type="hidden" name="socio" value="0" />
            <table width="90%" border="0" align="center" cellpadding="5" cellspacing="0">
              <tr>
                <td width="100" valign="top" class="txtBold">Nombre:</td>
		   ...
		   '.$this->showComments($_GET["tipo"], $row["id"]).'
...
public function add(){
	...
	if($socio=='2')
		$comentario = $_POST["comentarios2"];
	else
		$comentario = $_POST["comentarios"];
	$fechapub = date("Y-m-d");
	$link = $_POST["link"];
	$tipo = '0';
	$redireccion='firstMajestic&_f=main';
	if($socio=='2'){
		$socio='1';
		$tipo='2';
	}else
	...
	if($socio=='0'){
		$redireccion=''.$this->tabla[$seccion].'&_f=ver&id='. $id_publicacion;
		$socio='0';
		$tipo='0';
	}
	
	$db = $this->_db();
	mysql_query("INSERT INTO comentarios (seccion, id_publicacion, id_usuario, usuario, comentario, fechapub, activo, socio, tipo)
					VALUES ('". $seccion ."', '". $id_publicacion ."', '". $id_usuario ."', '". $usuario ."', '". $comentario ."', '". $fechapub ."', '0','".$socio."','".$tipo."')") or die(mysql_error());
	$id=mysql_insert_id();
	$echo ='<script type="text/javascript">
					setTimeout("alert(\'SU REGISTRO HA SIDO AGREGADO\');",100); 
					setTimeout("top.location.href = \'?F='. $redireccion .'\'",1000);
					</script>';
	//$this->enviar($seccion,$usuario,$comentario,$fechapub,$link);
	return $echo;
...
# *****************************************************************************************************************************************************************
private function enviar($seccion,$usuario,$comentario,$fechapu,$link){
	...
	$datos = '';
	$datos .= '<p>Te informamos que se a ingresado un comentado en el apartado: '.$apartado.', en la fecha: '.$fechapu.', por el usuario: '.$usuario.'.</p>';
	$datos .= '<p>Link: </p><span class="linkPP"><a href="'.top.location.href.'/'.$link.'"</a>http://www.outletminero.org/'.$link.'</span><p></p>';
