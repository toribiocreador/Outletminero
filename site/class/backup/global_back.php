...
public function showPublicacion($result, $table, $tipoComm){
	$content = '';
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)){		
		$imagen = (isset($row["imagen"]) && $row["imagen"] != '') ? '<td width="100" valign="top"><table width="100" border="0" cellpadding="0" cellspacing="0" class="tbImgs"><tr><td><a href="?F='. $this->seccion[$tipoComm] .'&amp;_f=ver&amp;id='. $row["id"] .'"><img src="'. $row["imagen"] .'" alt="" border="0" /></a><span class="txtPP">Foto: '. $row["pf"] .'</span></td></tr></table></td>' : '';
		...
		$content .= '<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tbTitNot">
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="10">
            <tr>
              '. $imagen .'
              <td valign="top" class="txtCont">'. $row["introduccion"] .'
...
public function showComments($seccion, $id_publicacion){
	$db = $this->_db();
	$result = mysql_query("SELECT * FROM comentarios WHERE seccion='{$seccion}' AND id_publicacion='{$id_publicacion}' ORDER BY id DESC");
...
public function listGiros($id){
	$echo = '<select name="giro" id="giro" class="frmInputM"><option value="0">== Seleccione un Giro ==</option>';
	$db = $this->_db();
