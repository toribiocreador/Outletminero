...
public function main(){
	...
	$result = mysql_query("SELECT * FROM historias WHERE socio='0' AND activo=1 ORDER BY id DESC ". $qLimit);
	$content .= '<p class="txtTit">Historias</p>';
	return $content . $this->showPublicacion($result, 'historias', '6') . $this->paginacion($_GET["ver"], $this->PAGINAS, $this->LIMIT, 'historias', 'historias', 'main', 6);
}//mai6

public function ver(){
	if (!isset($_GET["id"]) || $_GET["id"] == ''){ echo '<h2>No se encuentra el identificador</h2>'; exit(); }//if
	$this -> lectura("historias",$_GET["id"]);
	$db = $this->_db();
	$result = mysql_query("SELECT * FROM historias WHERE id='{$_GET['id']}' AND activo=1");
	$content = '';
